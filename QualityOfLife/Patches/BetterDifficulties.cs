...
namespace Marioalexsan.GrindeaQoL
{
    ...
    static class BetterDifficulties
    {
        ...
        [HarmonyPatch(typeof(Game1), nameof(Game1._InGameMenu_RenderSystem))]
        [HarmonyTranspiler]
        static IEnumerable<CodeInstruction> RenderSystem(IEnumerable<CodeInstruction> code)
        {
            // This is kinda hacky
            return code.Select(x => !x.LoadsConstant("--") ? x : new CodeInstruction(OpCodes.Ldstr, "Normal+"));
        }
